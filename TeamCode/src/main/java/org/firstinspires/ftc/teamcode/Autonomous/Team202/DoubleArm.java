...
public class DoubleArm extends Thread {
    ...
    private static double claw_target = 0.5;
    ...
    public void run() {
        while (should_be_running) {
            right_motor.setPower(
                    Math.max(Math.min(0.02 * (right_motor_target - right_motor.getCurrentPosition()), max_power[0]),
                            min_power[0]));
            left_motor.setPower(right_motor.getPower());

            joint2.setPower(Math.max(Math.min(0.02 * (joint_target - joint2.getCurrentPosition()), max_power[1]),
                    min_power[1]));
