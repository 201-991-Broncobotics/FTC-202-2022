...
public class AprilTag extends Thread {
    ...
    // shouldn't really matter, we don't really care about distance and azimuth.
    // Lens intrinsics
    // UNITS ARE PIXELS
    ...
    // size of the tag in meters (shouldn't really matter)
    ...
    /** @return the most counted april tag in all of the camera iterations. defaults to tag 1. */
    public int getMostDetected() {
        if (tag1count > tag2count && tag1count > tag3count) return tag1id;
        else if (tag2count > tag1count && tag2count > tag3count) return tag2id;
        else if (tag3count > tag1count && tag3count > tag2count) return tag3id;
...
// stolen from open ftc april tag example :)
